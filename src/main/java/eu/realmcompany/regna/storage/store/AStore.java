package eu.realmcompany.regna.storage.store;
...
import eu.realmcompany.regna.storage.data.FriendlyData;
...
public abstract class AStore {
    ...
    public void provideDefault() throws Exception {
        if (hasDefault) {
            // write default file
            try (Reader reader = new InputStreamReader(Objects.requireNonNull(instance.getResource(path))); Writer writer = new FileWriter(this.file)) {
                int r;
                do {
                    r = reader.read();
                    if (r != -1)
